...
install () {
    ...
    # install pionux package (this will always return failed exit status)
    dpkg -i pionux_* || true
...
install_minimal () {
    ...
    mv /home/pionux/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel-minimal.xml \
        /home/pionux/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml
    chown -R pionux:pionux /home/pionux/.config
}
...
add_pionux_key () {
    ...
    curl -fsSL https://pionuxos.com/static/gpg.txt | apt-key add -
}
...
# add pionux apt repository for installing dependencies
add_pionux_key
cat > /etc/apt/sources.list.d/pionuxos.list <<EOF
deb http://packages.pionuxos.com/debian testing/
EOF
...
# delete pionux apt repository for now (upgrades not tested)
rm /etc/apt/sources.list.d/pionuxos.list
